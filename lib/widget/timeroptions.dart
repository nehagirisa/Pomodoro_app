    ...
    return SingleChildScrollView(
      ...
      child: Row(
        children: selectableTimes.map((time) {
          return InkWell(
            onTap: () => provider.selectTime(double.parse(time.toString())),
            child: Container(
              margin: const EdgeInsets.only(left: 10),
              width: 70,
              height: 50,
              decoration: int.parse(time.toString()) == provider.selectedTime
                  ? BoxDecoration(
                      color: Colors.white,
                  ...
                  : BoxDecoration(
                      ...
                      borderRadius: BorderRadius.circular(15)),
              child: Center(
                child: Text((int.parse(time.toString()) ~/ 60).toString(),
                    style: int.parse(time.toString()) == provider.selectedTime
                        ? TextStyle(
                            fontSize: 20,
